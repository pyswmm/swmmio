...
class model(object):
	...
	def organizeConduitData (self, bbox=None, subset=None, extraData=None, findOrder=False):	
		...
		for conduit in conduitsDict:
			...
			#except:
				#print "problem with conduit: " + str(conduitsDict[conduit]) 
				#errCount += 1
...
class rpt(SWMMIOFile):
	...
	def createRptTrimedToDTimeRange (self,  timeSeriesUniqStartString = "JUL-"):
		
		#startDT = datetime.datetime.strptime(startDTime, "%b-%d-%Y %H:%M:%S")
		#endDT = datetime.datetime.strptime(endDTime, "%b-%d-%Y %H:%M:%S")
		
		newFile = os.path.join(self.dir, self.name + "_trimed.rpt") 
		
		#not ready for prime time
		with open(self.filePath) as existingF:
			 with open(newFile,"wb") as output: 
				for line in existingF:
					
					if not timeSeriesUniqStartString in line or "JUL-23-2013 02:" in line:
						output.write(line)
